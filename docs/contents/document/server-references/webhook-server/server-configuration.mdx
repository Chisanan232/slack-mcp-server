...
# Method 4: UV (Modern Package Manager)
...
slack-webhook-server --host 127.0.0.1 --port 3000  # Development
slack-webhook-server --host 0.0.0.0 --port 8080   # Production
slack-webhook-server --port $PORT                  # Cloud deployment
...
# Method 3: Poetry
poetry run slack-webhook-server --host 127.0.0.1 --port 3000

# Method 4: UV
uv run slack-webhook-server --host 0.0.0.0 --port 8080
```
...
```bash
# Method 1: Console Script
slack-webhook-server --log-level DEBUG    # Development
slack-webhook-server --log-level WARNING  # Production
slack-webhook-server --log-level DEBUG    # Troubleshooting
...
**Examples:**
```bash
# Method 1: Console Script (recommended - uses environment variables)
...
# Method 3: Poetry
export SLACK_BOT_TOKEN="xoxb-your-token-here"
poetry run slack-webhook-server

# Method 4: UV
export SLACK_BOT_TOKEN="xoxb-your-token-here"
uv run slack-webhook-server

# Alternative: Provide token via CLI (all methods, not recommended for production)
slack-webhook-server --slack-token "xoxb-your-token-here"
...
```bash
# Method 1: Console Script
slack-webhook-server --env-file /etc/webhook/.env.production
slack-webhook-server --env-file ../config/.env.staging
slack-webhook-server --no-env-file
...
# Method 3: Poetry
poetry run slack-webhook-server --env-file ../config/.env.staging

# Method 4: UV
uv run slack-webhook-server --env-file /etc/webhook/.env.production
```
...
```bash
# Method 1: Console Script
slack-webhook-server                                      # Standalone (default)
slack-webhook-server --integrated --mcp-transport sse    # Integrated with SSE
slack-webhook-server --integrated --mcp-transport streamable-http  # HTTP streaming
slack-webhook-server --integrated --mcp-mount-path /api/mcp  # Custom mount path
...
# Method 3: Poetry
poetry run slack-webhook-server --integrated --mcp-transport sse

# Method 4: UV
uv run slack-webhook-server --integrated --mcp-transport streamable-http
```
