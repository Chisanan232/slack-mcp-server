...
### Third-Party Queue Support

The following message queue systems are planned for future releases:

- 🚧 **Redis** - *Coming Soon* Redis backend with persistence
- 🚧 **Apache Kafka** - *Coming Soon* - High-throughput distributed streaming platform
- 🚧 **RabbitMQ** - *In Plan* - Robust message broker with advanced routing
- 🚧 **Amazon SQS** - *Under Consideration* - AWS Simple Queue Service integration
...
```bash
# Currently supported
export QUEUE_BACKEND=memory          # Default for development
export QUEUE_BACKEND=redis           # Production-ready option
export REDIS_URL=redis://localhost:6379
...
:::note Package Manager Compatibility
...
- **Poetry**: Replace `pip install` with `poetry add` and `python` with `poetry run python`
- **UV**: Replace `pip install` with `uv add` and `python` with `uv run python`
- **Pipenv**: Replace `pip install` with `pipenv install` and `python` with `pipenv run python`
:::
...
- **Deep Dive**: Review the [Architecture Documentation](/dev/next/architecture/event-handler-queue-architecture)
