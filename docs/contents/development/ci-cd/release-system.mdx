...
### 1. Production Release Workflow ([`release.yml`](https://github.com/Chisanan232/slack-mcp-server/blob/master/.github/workflows/release.yml))
...
graph TD
    ...
    C --> D[Check Dockerfile]
    D -->|Exists| E[Enable Docker Jobs]
    D -->|Missing| F[Skip Docker Jobs]
    
    C --> G[Version Bump]
    B --> H[Config Outputs]
    H --> I[Docker Registry URLs]
    H --> J[Git Commit Settings]  
    H --> K[Documentation Paths]
    ...
    L --> O[PyPI via OIDC]
    M --> P[Docker Hub - Config Based]
    M --> Q[GitHub Container Registry - Config Based]
    N --> R[Docusaurus Versioning - Config Based]
    
    L & M & N --> S[Supply Chain Security]
    S --> T[SBOM Generation]
    S --> U[Cosign Signing]  
    S --> V[SLSA Attestation]
```
...
> 📋 **Template Workflow**: [`rw_parse_project_config.yaml`](https://github.com/Chisanan232/Template-Python-UV-Project/blob/master/.github/workflows/rw_parse_project_config.yaml)

- Parses enhanced [`intent.yaml`](https://github.com/Chisanan232/slack-mcp-server/blob/master/.github/tag_and_release/intent.yaml) with project, docker, git, validation, and docs configuration
- Provides structured outputs for all downstream jobs
- Auto-detects project name and package name from repository and [`pyproject.toml`](https://github.com/Chisanan232/slack-mcp-server/blob/master/pyproject.toml)
...
> 📋 **Template Workflow**: Integrated into [`rw_release_complete.yaml`](https://github.com/Chisanan232/Template-Python-UV-Project/blob/master/.github/workflows/rw_release_complete.yaml)
...
> 📋 **Template Workflow**: [`rw_parse_release_intent.yaml`](https://github.com/Chisanan232/Template-Python-UV-Project/blob/master/.github/workflows/rw_parse_release_intent.yaml)
...
All release workflows now automatically detect the presence of a `Dockerfile` in the repository root:

- ✅ **Dockerfile exists**: Docker processes run based on configuration (`auto`, `force`, `skip`)
- ⚠️ **No Dockerfile**: Docker processes are automatically skipped with clear logging
- 📋 **Summary output**: Workflow summaries clearly indicate when Docker was skipped

**Affected Workflows:**
- **Production Release**: Skips Docker Hub and GHCR publishing
- **Validation**: Skips Docker build validation and security scanning  
- **Staging**: Skips Docker Hub and GHCR RC image publishing
...
This allows the template workflows to work seamlessly for Python-only projects.
...
2. **Docker Image** (`GHCR` & `DockerHub`) - **Config Enhanced**:
   - **Automatic Dockerfile Detection**: Conditionally executes based on Dockerfile presence
   ...
   - **Supply Chain Security**: SBOM generation, Cosign signing, SLSA attestation (when Dockerfile exists)
...
graph TD
    ...
    C --> D[Check Dockerfile]
    D -->|Exists| E[Enable Docker Validation]
    D -->|Missing| F[Skip Docker Validation]
    
    C --> G[Python Build Check]
    C --> H[Docker Build & Test - Config Based]
    C --> I[Docs Build Test - Config Based]
    C --> J[Supply Chain Security]
    ...
    B --> K[Config Validation]
    K --> L[Registry URL Validation]
    K --> M[Documentation Path Validation]
    K --> N[Git Config Validation]
    
    J --> O[SBOM Generation]
    J --> P[Vulnerability Scanning]
    J --> Q[Security Attestation]
    
    G & H & I & J & K --> R[Enhanced Validation Summary]
```
...
> 📋 **Template Workflow**: Integrated into [`rw_release_validation_complete.yaml`](https://github.com/Chisanan232/Template-Python-UV-Project/blob/master/.github/workflows/rw_release_validation_complete.yaml)
...
- **Dockerfile Detection**: Automatically skips Docker security scans when no Dockerfile present
- **Defensive Checks**: Pre-flight image existence validation
- **Vulnerability Scanning**: Grype integration for CVE detection (when Dockerfile exists)
- **Fallback SBOM**: Workspace scanning if Docker image scan fails
- **Supply Chain Security**: Full SBOM, signing, and attestation testing (when Dockerfile exists)

### 3. Staging Release Workflow ([`release-staging.yml`](https://github.com/Chisanan232/slack-mcp-server/blob/master/.github/workflows/release-staging.yml))
...
graph TD
    ...
    C --> D[Check Dockerfile]
    D -->|Exists| E[Enable Docker Staging]
    D -->|Missing| F[Skip Docker Staging]
    
    C --> G[Staging Python Package]
    C --> H[Staging Docker Images]
    C --> I[Staging Documentation]
    
    E --> H
    F -.->|Skipped| H
    
    G --> J[TestPyPI - Config Based]
    H --> K[GHCR Staging - Config Based]
    I --> L[Preview Branch - Config Based]
    
    B --> M[Config Outputs]
    M --> N[Validation Version Settings]
    M --> O[Custom Preview Branch]
    M --> P[Registry URLs]
```
...
> 📋 **Template Workflow**: Integrated into [`rw_release_staging_complete.yaml`](https://github.com/Chisanan232/Template-Python-UV-Project/blob/master/.github/workflows/rw_release_staging_complete.yaml)

- Checks Dockerfile presence before staging Docker operations
- Ensures staging workflow works for Python-only projects
...
2. **GHCR Staging** - **Config Enhanced**: 
   - Docker images with `-rc` tags using `${{ needs.config.outputs.docker_registry_ghcr }}` (when Dockerfile exists)
...
**🔄 Release Workflows**: [Template Release System Docs](https://chisanan232.github.io/Template-Python-UV-Project/dev/next/ci-cd/release-system)  
...
**💡 About Template Release Workflows**: The Template-Python-UV-Project provides production-ready release workflows including configuration parsing, release intent validation, multi-target deployments, and comprehensive supply chain security features.
...
✅ **Template Integration**: Leverages battle-tested workflows from [Template Project](https://github.com/Chisanan232/Template-Python-UV-Project)
...
:::tip Template Repository Reference
[`Chisanan232/Template-Python-UV-Project`](https://github.com/Chisanan232/Template-Python-UV-Project) | [📖 Documentation](https://chisanan232.github.io/Template-Python-UV-Project/dev/next/ci-cd)
:::
...
The `docker` configuration setting is now enhanced with automatic Dockerfile detection. Even if you set `docker: force`, the workflow will gracefully skip Docker processes if no Dockerfile is found in the repository root. This ensures the same workflows work for both Docker-based and Python-only projects.
:::
...
The `docker` setting now works in conjunction with automatic Dockerfile detection:
...
**Note**: Docker processes are automatically skipped when no Dockerfile is present, regardless of the configuration setting.
...
**Q: Why are Docker jobs being skipped even though I have `docker: force` configured?**

A: The workflows now automatically detect if a `Dockerfile` exists in your repository root. If no Dockerfile is found, all Docker-related processes are gracefully skipped regardless of configuration. This is intentional to support Python-only projects. If you expect Docker jobs to run:

1. Verify a `Dockerfile` exists in your repository root
2. Check the "Dockerfile Check" step in the workflow logs
3. Ensure the Dockerfile is committed to your branch
