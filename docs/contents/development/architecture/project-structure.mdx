...
│   ├── handler/               # Event handler implementations
...
:::note Queue Backend Abstraction
The queue backend abstraction layer has been migrated to the **[abstract-backend](https://github.com/Chisanan232/abstract-backend)** library. This library provides a unified interface for message queue backends (Memory, Redis, Kafka, etc.).

For queue backend implementation details, plugin development, and usage examples, please refer to the **[abstract-backend documentation](https://chisanan232.github.io/abstract-backend/)**.
:::
...
Queue backend implementations are now managed by the **[abstract-backend](https://github.com/Chisanan232/abstract-backend)** library.

To add new queue backends:
1. Refer to the **[abstract-backend plugin development guide](https://chisanan232.github.io/abstract-backend/)**
2. Implement the `MessageQueueBackend` protocol from `abe.backends.message_queue`
3. Register your backend as an entry point in your plugin package
4. Install your backend plugin alongside `slack-mcp`
:::
