...
class AdvancedSlackEventHandler(BaseSlackEventHandler):
    ...
    async def on_message__channels(self, event: Dict[str, Any]) -> None:
        ...
        if "<@" in text:
            mentions = [item.split("<@")[1].split(">")[0] for item in text.split() if item.startswith("<@") and item.endswith(">")]
...
async def main():
    ...
    try:
        ...
        await asyncio.gather(
            oo_consumer.run(),
            decorator_consumer.run()
        )
    except KeyboardInterrupt:
        logger.info("Shutting down gracefully...")
        await asyncio.gather(
            oo_consumer.shutdown(),
            decorator_consumer.shutdown(),
