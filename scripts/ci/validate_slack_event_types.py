...
KNOWN_MESSAGE_SUBTYPES = ["message.app_home", "message.channels", "message.groups", "message.im", "message.mpim"]
...
def compare_events(
    ...
    if not (missing_in_enum_standard or missing_in_enum_subtype or extra_in_enum_standard or extra_in_enum_subtype):
...
def validate_enum_completeness(
    ...
    if missing_standard or missing_subtype:
        error_msg_parts.append("❌ SlackEvent enum is missing events from the API specification:")
        if missing_standard:
    ...
    if strict and (extra_standard or extra_subtype):
        if not error_msg_parts:  # If this is the first error, add header
            error_msg_parts.append("⚠️  SlackEvent enum contains events not present in the API specification:")
        else:
...
def format_output(event_types: Set[str], subtype_events: Set[str], output_format: str) -> str:
    """
...
def main() -> None:
    ...
    parser.add_argument(
        "--output", choices=["enum", "json", "list"], default="enum", help="Output format (default: enum)"
    )
    parser.add_argument(
        "--compare", action="store_true", help="Compare extracted events with current SlackEvent enum implementation"
    )
    parser.add_argument(
        "--validate",
        action="store_true",
    ...
    parser.add_argument(
        "--generate-update", action="store_true", help="Generate code to update SlackEvent enum with any missing events"
    )
    ...
    print(
        f"📊 Found {len(api_standard_events)} standard event types and {len(api_subtype_events)} subtype events "
    ...
    if args.compare or args.validate or args.generate_update:
        ...
        if enum_standard_events or enum_subtype_events:
            if args.compare:
                compare_events(api_standard_events, api_subtype_events, enum_standard_events, enum_subtype_events)

            if args.validate:
                validation_success = validate_enum_completeness(
                    api_standard_events, api_subtype_events, enum_standard_events, enum_subtype_events, args.strict
                )

            if args.generate_update:
                update_code = generate_update_code(
                    api_standard_events, api_subtype_events, enum_standard_events, enum_subtype_events
                )
