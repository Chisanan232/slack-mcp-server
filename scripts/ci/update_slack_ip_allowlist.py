...
class SlackAPIClient:
    ...
    def get_app_info(self, app_id: str) -> Dict[str, Any]:
        ...
        """
        try:
            response = self.client.api_call(api_method="admin.apps.approved.list", params={})
    ...
    def get_current_allowlist(self, app_id: str) -> Tuple[bool, List[Dict[str, str]], Optional[str]]:
        ...
        try:
            ...
            response = self.client.api_call(api_method="admin.apps.restrictions.list", params={})
            ...
            for app in response.get("restricted_apps", []):
                if app.get("app_id") == app_id:
                    for ip_range in app.get("allowed_ip_ranges", []):
                        ip_ranges.append(
                            {"cidr": ip_range.get("cidr", ""), "description": ip_range.get("description", "")}
                        )
                    break
    ...
    def update_ip_allowlist(self, app_id: str, ip_ranges: List[str]) -> bool:
        ...
        try:
            ...
            for idx, ip_range in enumerate(ip_ranges):
                formatted_ip_ranges.append({"cidr": ip_range, "description": f"GitHub Actions runner IP {idx+1}"})
            ...
            response = self.client.api_call(
                ...
                json={"app_id": app_id, "request_id": app_id, "allowed_ip_ranges": formatted_ip_ranges},
            )
...
def main() -> None:
    ...
    parser.add_argument(
        "--app-id", help="Slack App ID (default: from SLACK_APP_ID env var)", default=os.environ.get("SLACK_APP_ID")
    )
    parser.add_argument(
        "--token",
        help="Slack App Token (default: from SLACK_APP_TOKEN env var)",
        default=os.environ.get("SLACK_APP_TOKEN"),
    )
    parser.add_argument("--show-current", action="store_true", help="Show current IP allowlist without updating")
