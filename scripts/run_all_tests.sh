...
elif echo "$testing_type" | grep -q "contract-test";
then
    test_path=$(bash "$Scripts_Dir/$Test_Running_Script" ./test/contract_test/ windows | sed "s/\"//g" | sed 's/^//')
...
then
    ...
    contract_test_path=$(bash "$Scripts_Dir/$Test_Running_Script" ./test/contract_test/ windows | sed "s/\"//g" | sed 's/^//')
    test_path="$unit_test_path $integration_test_path $system_test_path $contract_test_path"
else
    test_path='error'
fi
