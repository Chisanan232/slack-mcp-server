...
def test_auto_select_non_memory_backend(reset_env):
    ...
    mock_entry_points = [
        MockEntryPoint("memory", memory_backend),
        MockEntryPoint("kafka", kafka_backend)
    ]
...
def test_fallback_to_memory_backend(reset_env):
    ...
    with patch('slack_mcp.backends.loader.entry_points') as mock_entry_points, \
        ...
        mock_warnings.warn.assert_called_with("No external backend found — using MemoryBackend (dev only).", UserWarning)
...
def test_legacy_no_backends_available(reset_env):
    ...
    with patch("slack_mcp.backends.loader.entry_points", return_value=[]), \
        ...
        # Verify warnings
        mock_warnings.warn.assert_any_call("No queue backends registered. Using MemoryBackend (development only).", UserWarning)
