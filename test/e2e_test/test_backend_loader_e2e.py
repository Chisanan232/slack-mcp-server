...
def test_memory_backend_fallback_e2e(reset_env):
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        assert any(("No queue backends" in text) or 
                  ("No external backend found" in text) for text in warning_texts)
...
def test_nonexistent_backend_error_message_e2e(reset_env):
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        try:
            ...
            warning_texts = [str(warning.message) for warning in w]
            assert any(
                "Unknown backend" in text or 
                "No queue backends registered" in text or
                "No external backend found" in text 
