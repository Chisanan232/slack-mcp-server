...
class TestQueueBackendProtocol:
    ...
    def test_protocol_methods(self):
        ...
        # Get the methods defined on the QueueBackend protocol
        protocol_methods = {
            name for name, _ in inspect.getmembers(QueueBackend, predicate=inspect.isfunction)
            if not name.startswith('_')
        }
        
        # With @runtime_checkable Protocol, classmethods aren't directly visible through inspection
        # So we check only the instance methods
    ...
    def test_method_signatures(self):
        ...
        # Skip testing from_env signature as it's a classmethod in a Protocol
        # which doesn't expose its signature through normal inspection methods
    ...
    @pytest.mark.asyncio
    async def test_from_env_usage(self):
        """Test that implementations are expected to provide from_env class method."""
        # Create a backend class that satisfies the protocol
        backend_class = MockQueueBackend
        
        # Verify it has a from_env class method
        assert hasattr(backend_class, "from_env")
        
        # Check that it returns an instance of the class
        instance = backend_class.from_env()
        assert isinstance(instance, backend_class)
        
        # Check that the instance works as expected
        await instance.publish("test_key", {"test": "value"})
